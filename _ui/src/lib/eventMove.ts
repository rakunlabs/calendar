...
import { dateInput, dayKey, toInstant } from './calendar';
import { shiftEditorDate } from './editorDates';
import { editOccurrence, editSeries, editorTimingZone, hasExceptions } from './recurrence';

export function movedEvent(master: CalendarEvent, occurrence: CalendarEvent, target: Date, end?: Date) {
  ...
  if (occurrence.all_day) {
    ...
    edited.date_to = toInstant(end ? dayKey(end) : shiftEditorDate(to, days), occurrence.tz || 'UTC');
  } else {
    const duration = Date.parse(occurrence.date_to) - Date.parse(occurrence.date_from);
    edited.date_from = target.toISOString();
    edited.date_to = (end ?? new Date(target.getTime() + duration)).toISOString();
  }
  if (Date.parse(edited.date_to) <= Date.parse(edited.date_from))
